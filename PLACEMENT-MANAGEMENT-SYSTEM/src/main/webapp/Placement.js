/* =============================================================
   Placement.js  —  All JS for Placement.jsp
   ============================================================= */
...
document.addEventListener("DOMContentLoaded", function() {
  buildModal();   // build modal FIRST before anything else
  fetchJobs();
  initFilters();
  initSearch();
});
  ...
  fetch("FetchJobsServlet")
    ...
    .catch(function() {
      getContainer().innerHTML =
        '<div style="color:red;text-align:center;width:100%">Error loading jobs. Please try again later.</div>';
    });
}
...
function renderCards(jobs) {
  ...
  if (!jobs.length) {
    container.innerHTML =
      '<div style="color:#333;text-align:center;width:100%">No jobs found.</div>';
    return;
  }
  ...
  jobs.forEach(function(job) {
    ...
    card.innerHTML =
      ...
      "<p class='department'>" + esc(job.department) + "</p>" +
      "<div class='tags'>" +
        "<span><i class='fa-solid fa-location-dot'></i> " + esc(job.location_type) + "</span>" +
        "<span><i class='fa-solid fa-briefcase'></i> " + esc(job.employment_type) + "</span>" +
        "<span><i class='fa-solid fa-money-bill-wave'></i> " + esc(job.salary_range) + "</span>" +
      "</div>" +
...
// ── 5. Filter + Search combined ────────────────────────────────
function applyFilters() {
  var searchInput = document.querySelector(".search-box input");
  var query = searchInput ? searchInput.value.toLowerCase().trim() : "";

  var filtered = allJobs.filter(function(job) {
    var dept  = (job.department || "").toLowerCase();
    var title = (job.job_title  || "").toLowerCase();
    var matchesDept   = activeFilter === "All" || dept === activeFilter.toLowerCase();
    var matchesSearch = !query || title.indexOf(query) !== -1 || dept.indexOf(query) !== -1;
...
// ── 6. Modal ───────────────────────────────────────────────────
function buildModal() {
  // Avoid duplicate modal if function somehow called twice
  ...
  // Now safe to query — elements exist in DOM
...
function openModal(job) {
  ...
  document.getElementById("modal-tags").innerHTML =
    "<li><i class='fa-solid fa-location-dot'></i> &nbsp;" + esc(job.location_type) + "</li>" +
    "<li><i class='fa-solid fa-briefcase'></i> &nbsp;" + esc(job.employment_type) + "</li>" +
    "<li><i class='fa-solid fa-money-bill-wave'></i> &nbsp;" + esc(job.salary_range) + "</li>";
...
function submitApplication(title) {
  // TODO: replace with real servlet call:
  // fetch("ApplyServlet", { method:"POST", body: new URLSearchParams({ jobTitle: title }) })
  ...
  setTimeout(function() {
    ...
    btn.style.background = "linear-gradient(135deg,#0df,#0056b3)";
    btn.disabled         = false;
  }, 1800);
}
...
function getContainer() {
  return document.getElementById("job-cards-container");
}
